...
bool esp8266_Reset()
{
	...
	if(HAL_UART_Transmit(&huart2, AT_RESET, sizeof(AT_RESET) - 1, HAL_MAX_DELAY) == HAL_OK)
	{
		if(HAL_UART_Receive(&huart2, (uint8_t*)tmp, sizeof(tmp), HAL_MAX_DELAY) == HAL_OK)
		{
				if(tmp[11] == 'O' && tmp[12] == 'K')
				{
					 HAL_Delay(2000);
					return true;
				}
...
bool esp8266_DisconnectAP()
{
	...
	if(HAL_UART_Transmit(&huart2, AT_DISCONNECT_AP, sizeof(AT_DISCONNECT_AP), HAL_MAX_DELAY) == HAL_OK)
	{
		if(HAL_UART_Receive_IT(&huart2, (uint8_t*)tmp, sizeof(tmp)) == HAL_OK)
		{
			HAL_Delay(1000);
...
bool esp8266_SendTCP(uint8_t* data, int length)
{
	if(length != 0)
	{
		...
		//memcpy(SendData + length, "\r\n", 2);
		sprintf((&SendData[length]), "\r\n", 2);
